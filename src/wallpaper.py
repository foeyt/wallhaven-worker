...
class WallHaven:
    def __init__(self):
        self.url = "https://wallhaven.cc/search?categories=110&purity=100&ratios=16x9&sorting=random&order=desc&seed=dRIyiE&page=" + str(random.randint(2, 8056))
        self.headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3"
        }

    def get_page(self, url):
        page = requests.get(url, headers=self.headers, timeout=30).content.decode("utf-8")
        return page
